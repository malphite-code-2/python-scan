...
from hdwallet.symbols import RVN as SYMBOL
...
async def get_balance(wallets):
    try:
        urls = [{'url': f"https://ravencoin.atomicwallet.io/api/v2/address/{wallet['address']}", 'address': wallet['address']} for wallet in wallets]
...
async def seek(i) :
    ...
    while True :
        ...
        for ck in wallets:
            ...
            print(Fore.GREEN , f"[CPU{i}][C: {z} / W: {w}]", Fore.BLUE , f"{str(address)} - {str(seed)}" , Fore.RED ,' [' + str(balance) +' RVN]' , Fore.WHITE , Style.RESET_ALL)
            ...
            if balance > 0 or address in add:
                ...
                f.close()
                print(Fore.MAGENTA , 'Information File Name ========> winner-rvn.txt [OK]')
                message('NEW RVN WALLET IS FOUND!', f"[{balance} RVN] \n Address: [{address}] \n Seed: [{seed}] \n Private: [{private_key}]")
                continue
